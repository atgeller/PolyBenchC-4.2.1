...
echo "name,mean_1,sem_1,mean_2,sem_2,mean_3,sem_3" > benchmark_data.csv
echo "name,mean_1,sem_1,mean_2,sem_2,mean_3,sem_3" > benchmark_data_small.csv
...
do
    ...
    >&2 echo Running
    eval "hyperfine --export-json temp/$(echo $benchmark)_no_checks.json $(printf "'~/wasmtime_no_checks_at_all/target/release/wasmtime --allow-precompiled %s/%s_no_checks.cwasm'" $path $benchmark)" >> /dev/null
